...
def load(image_path):
    ...
    ##out = skimage.io.imread(image_path) 错误写法，因为没有引入skimage
...
def mix_images(image1, image2, channel1, channel2):
    ...
    out = None
    ### YOUR CODE HERE
    image1_ex = rgb_exclusion(image1,channel1)
    image2_ex = rgb_exclusion(image2,channel2)
    left = image1_ex[:, :np.shape(image1_ex)[1]//2, :]
    right = image2_ex[:, np.shape(image2_ex)[1]//2: ,:]
...
def mix_quadrants(image):
    ...
    ### YOUR CODE HERE
    image_ex1 = rgb_exclusion(image,"R")
    image_ex2 = dim_image(image)
    image_ex3 = image**0.5
    image_ex4 = rgb_exclusion(image,"R")
    Top_left = image_ex1[:np.shape(image_ex1)[0]//2, :np.shape(image_ex1)[1]//2, :]
    Top_right = image_ex2[:np.shape(image_ex2)[0]//2, np.shape(image_ex2)[1]//2:, :]
    Bottom_left = image_ex3[np.shape(image_ex3)[0]//2:, :np.shape(image_ex3)[1]//2, :]
    Bottom_right = image_ex4[np.shape(image_ex4)[0]//2:, np.shape(image_ex4)[1]//2:, :]
